    ...
    <div class="main-content-inner">
        <div class="main-content-wrap">
            ...
            <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                {{-- Khối hiển thị ảnh hiện tại và dropdown chọn màu, nằm trong form chính --}}
                @if ($product->images->isNotEmpty())
                    <div class="wg-box mb-3">
                        <h5 class="body-title">Ảnh hiện tại</h5>
                        <div class="flex flex-wrap gap10">
                            @foreach ($product->images as $img)
                                <div class="border rounded p-1 text-center d-flex flex-column align-items-center"
                                     style="position: relative; width: 140px; height: auto;">
                                    <img src="{{ asset('storage/' . $img->image) }}"
                                         style="width: 120px; height: 120px; object-fit: cover; border-radius: 6px;">

                                    {{-- Dropdown này nằm trong form chính để gửi dữ liệu lên server --}}
                                    <div class="mt-2" style="width: 100%;">
                                        <select name="existing_images[{{ $img->id }}][color_id]" class="form-control"
                                                style="width: 100%;">
                                            <option value="">Chọn màu</option>
                                            @foreach ($colors as $color)
                                                <option value="{{ $color->id }}"
                                                        {{ old('existing_images.' . $img->id . '.color_id', $img->color_id) == $color->id ? 'selected' : '' }}>
                                                    {{ $color->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    {{-- Nút xóa ảnh này KHÔNG nằm trong form cập nhật --}}
                                    <button type="button" 
                                            onclick="if(confirm('Bạn chắc chắn muốn xóa ảnh này?')) { document.getElementById('delete-image-{{ $img->id }}').submit(); }"
                                            style="position: absolute; top: 4px; right: 4px; margin: 0; padding: 0; background: none; border: none;">
                                        <div style="background: rgba(255,255,255,0.8); border: none; border-radius: 50%; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; box-shadow: 0 1px 4px #0001; cursor: pointer;">
            ...
            {{-- Các form riêng biệt cho việc xóa ảnh, được đặt ngoài form chính --}}
            @if ($product->images->isNotEmpty())
                @foreach ($product->images as $img)
                    <form id="delete-image-{{ $img->id }}" action="{{ route('admin.products.image.destroy', $img->id) }}" method="POST" style="display: none;">
                        @csrf
                        @method('DELETE')
                    </form>
                @endforeach
            ...
            if (input.files) {
                Array.from(input.files).forEach((file, index) => {
                    ...
                    reader.onload = function(e) {
                        ...
                        imageContainer.className = 'd-flex flex-column align-items-center mb-2';
                        imageContainer.style = 'width: 140px; height: auto; border: 1px solid #ccc; padding: 5px; border-radius: 8px;';

                        const img = document.createElement('img');
                        img.src = e.target.result;
                        img.style = 'max-width: 120px; max-height: 120px; border-radius: 8px; object-fit: cover;';

                        const select = document.createElement('select');
                        select.name = `new_images[${index}][color_id]`;
                        select.className = 'form-control mt-2';
                        select.style = 'width: 100%';

                        const defaultOption = document.createElement('option');
                        defaultOption.value = '';
                        defaultOption.textContent = 'Chọn màu';
                        ...
                        imageContainer.appendChild(select);
