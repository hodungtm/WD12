                                ...
                                <select name="products[0][variant_id]" class="form-control variant-select" required>
                                    ...
                                    @foreach($products as $product)
                                        @foreach($product->variants as $variant)
                                            <option value="{{ $variant->id }}" data-product="{{ $product->name }}" {{-- Lấy giá đã
                                                trừ khuyến mãi: ưu tiên sale_price nếu> 0, ngược lại là price --}}
                                                ...
                                                {{-- Hiển thị tên sản phẩm, size và màu sắc --}}
                                                {{ $product->name }} - Size: {{ $variant->size->name ?? '' }} - Màu:
                                                {{ $variant->color->name ?? '' }}
