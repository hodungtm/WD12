        ...
        <div class="col-md-12">
            <div class="tile">
                ...
                <div class="d-flex align-items-center justify-content-start mb-3" style="gap: 10px;">
                    <a href="{{ route('posts.create') }}" class="btn btn-success btn-sm d-flex align-items-center" style="gap: 5px;">
                        <i class="fas fa-plus"></i> Tạo mới bài viết
                    </a>
                    <a href="" class="btn btn-warning btn-sm d-flex align-items-center" title="Tải từ file" style="gap: 5px;">
                    ...
                    <button type="button" class="btn btn-secondary btn-sm d-flex align-items-center js-textareacopybtn" style="gap: 5px;">
                    ...
                    <form action="" method="POST" onsubmit="return confirm('Bạn có chắc muốn xóa tất cả?');" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-secondary btn-sm d-flex align-items-center" style="background-color: #6c757d; border-color: #6c757d; color: white; gap: 5px;">
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        ...
                        <select name="per_page"  class="form-control-sm" onchange="this.form.submit()" style="width: auto;">
        ...
        <tbody>
            @forelse ($posts as $post)
                <tr class="text-center align-middle">
                    <td><input type="checkbox" name="selected_posts[]" value="{{ $post->id }}" class="check_item"></td>
                    <td>{{ $post->id }}</td>
                    <td class="text-start">{{ $post->title }}</td>
                    <td>
                        @if ($post->image)
                            <img src="{{ asset('storage/' . $post->image) }}" alt="Ảnh" width="100" class="img-thumbnail">
                        @else
                    ...
                    <td>
                        <a href="{{ route('posts.show', $post) }}" class="btn btn-info btn-sm" title="Xem"><i class="fas fa-eye"></i></a>
                        <a href="{{ route('posts.edit', $post) }}" class="btn btn-warning btn-sm" title="Sửa"><i class="fas fa-edit"></i></a>
