    ...
    <div class="tile-body">
      <form action="{{ route('Admin.reviews.store') }}" method="POST">
        ...
        <div class="mb-3">
          ...
          <input type="number" name="user_id" id="user_id" value="{{ old('user_id') }}" class="form-control" placeholder="ID người dùng (có thể để trống)">
        </div>

        <div class="mb-3">
          <label for="product_id" class="form-label">ID Sản phẩm</label>
          <input type="number" name="product_id" id="product_id" value="{{ old('product_id') }}" class="form-control" placeholder="ID sản phẩm (có thể để trống)">
        </div>

        <div class="mb-3">
          <label for="so_sao" class="form-label">Số sao (1-5)</label>
          <input type="number" name="so_sao" id="so_sao" value="{{ old('so_sao') }}" class="form-control" min="1" max="5" required>
        </div>
