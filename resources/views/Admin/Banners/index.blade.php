  ...
  <div class="main-content-wrap" style="padding-top: 0; margin-top: 0;">
    ...
    <div class="wg-box">
      ...
      <div class="flex items-center gap10 mb-3" style="color:#1abc9c; font-size:16px;">
        ...
        <span>Tip: Bạn có thể tìm kiếm theo <b>ID</b> hoặc <b>tiêu đề</b> để lọc nhanh banner.</span>
      </div>

      <form method="GET" action="{{ route('admin.banners.index') }}" class="flex flex-wrap gap-3 mb-4 align-items-center">
        <div class="flex items-center gap10">
          ...
          <select name="per_page" id="per_page" class="form-select" style="width: 70px;" onchange="this.form.submit()">
            @foreach([5, 10, 20] as $num)
              <option value="{{ $num }}" {{ request('per_page', 5) == $num ? 'selected' : '' }}>{{ $num }}</option>
            @endforeach
          </select>
          <span class="text-tiny" style="color:#222;">dòng</span>
        </div>
        <input type="text" name="search" class="form-control" placeholder="Tìm tiêu đề..." value="{{ request('search') }}" style="min-width: 200px; max-width: 300px;">
        ...
        <ul class="user-item flex gap0 mb-0 table-row-align" style="border-bottom:1px solid #f2f2f2;">
          ...
          <li class="col-images">
            @if ($banner->hinhAnhBanner && $banner->hinhAnhBanner->count())
              @foreach ($banner->hinhAnhBanner as $img)
                <img src="{{ asset('storage/' . $img->hinh_anh) }}" alt="ảnh banner" style="max-width: 60px; max-height: 60px; border-radius: 4px;">
              @endforeach
            @else
          ...
          <li class="col-status">
            @if($banner->trang_thai === 'hien')
              <span class="badge-status badge-instock">Hiển</span>
            @else
              <span class="badge-status badge-outstock">Ẩn</span>
            @endif
          </li>
          <li class="col-action">
            <a href="{{ route('admin.banners.show', $banner->id) }}" title="Xem" class="action-icon"><i class="icon-eye"></i></a>
            <a href="{{ route('admin.banners.edit', $banner->id) }}" title="Sửa" class="action-icon edit"><i class="icon-edit-3"></i></a>
            <form action="{{ route('admin.banners.destroy', $banner->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Bạn có chắc chắn muốn xóa banner này?');">
              @csrf
              @method('DELETE')
              <button type="submit" title="Xóa" class="action-icon" style="color: #e74c3c;"><i class="icon-trash-2"></i></button>
            </form>
          </li>
        </ul>
        @empty
...
.col-title   { flex: 1.8 0 140px; min-width: 140px; justify-content: flex-start; }
.col-images  { flex: 2.5 0 200px; min-width: 200px; justify-content: flex-start; display: flex; gap: 6px; flex-wrap: wrap; }
.col-type    { flex: 1.2 0 140px; min-width: 140px; justify-content: flex-start; }
...
@media (max-width: 1200px) {
    ...
    .col-images { min-width: 150px; }
}
@media (max-width: 900px) {
    .table-title li, .user-item li { font-size:15px; }
    .col-images { min-width: 120px; }
}
